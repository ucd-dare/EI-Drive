...
class DualControl(object):
    ...
    def _parse_vehicle_wheel(self):
        ...
        # if self.speed < 0.5 and throttleCmd > 0.5:
        #     self.starting.play(-1)
        if 0.5 < throttleCmd:
            # self.starting.stop()
            self.acceleration.play(-1)
        else:
            # self.starting.stop()
    ...
    def tick(self, world, clock):
        ...
        if isinstance(c, carla.VehicleControl):
            self._info_text += [
                ...
                'Gear:                   %s' % {-1: 'R', 0: 'N'}.get(c.gear, c.gear)]  # 11 space added
...
def get_camera_view(vehicle_model):
    ...
    elif 'vehicle.citroen.c3' == vehicle_model:
        return carla.Transform(carla.Location(x=-0.22, y=-0.35, z=1.3), carla.Rotation(pitch=5))
    elif 'vehicle.toyota.prius' == vehicle_model:
        return carla.Transform(carla.Location(x=-0.12, y=-0.35, z=1.2), carla.Rotation(pitch=5))
    elif 'vehicle.dodge.charger_police_2020' == vehicle_model:
        return carla.Transform(carla.Location(x=-0.12, y=-0.35, z=1.2), carla.Rotation(pitch=0))
    else:
        return carla.Transform(carla.Location(x=-0.23, y=-1.3, z=1.3), carla.Rotation(pitch=0))
...
class MusicPlayer(object):
    def __init__(self, is_muted=True, loop=-1, start=0.0, fading_time=0):
        if '1' == is_muted:
            mixer.init()
            self._loading()
            self._play(loop, start, fading_time)
        else:
            print("Background Music is muted!")
...
def game_loop(args):
    ...
    MusicPlayer(args.music, -1, 0.0, 2)
...
class Gauge:
    ...
    def render(self, speed):
        ...
        if self.glow:
            ...
            for i in range(0, 15):
                ac[3] = int(150 - i * 10)
                pygame.gfxdraw.arc(self.screen, int(self.x_cord), int(self.y_cord), self.radius - self.thickness - i,
                                   -225, fill_angle - 225 - 8, ac)
            ...
            for i in range(0, 10):
                ac[3] = int(150 - i * 15)
                pygame.gfxdraw.arc(self.screen, int(lx), int(ly), (self.thickness // 2) + i, fill_angle - 225 - 10,
...
def main():
    ...
    argparser.add_argument(
        ...
        metavar='fullscreen',
        default='fullscreen',
        help='fullscreen or window (default: fullscreen)')
    argparser.add_argument(
        '--filter',
        metavar='PATTERN',
        default='vehicle.*',
    ...
    argparser.add_argument(
        ...
        metavar='M',
        default=True,
        help='Music Player (default: True)')
