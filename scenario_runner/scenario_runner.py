...
class ScenarioRunner(object):
    ...
    def _get_scenario_class_or_fail(self, scenario):
        ...
        scenarios_list = glob.glob("{}scenario_runner/srunner/scenarios/*.py")
        ...
        if not self._args.waitForEgo:
            ...
            while ego_vehicle_missing:
                ...
                for ego_vehicle in ego_vehicles:
                    ...
                    #carla_vehicles = self.world().get_actors().filter('vehicle.*')
                    carla_vehicles = CarlaDataProvider.get_world().get_actors().filter('vehicle.*')

                    print(str(len(carla_vehicles)))
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        if CarlaDataProvider.is_sync_mode():
            print("wait for tick sync", self.world, type(self.world))
            self.world.tick()
            print("wait for tick sync finsihed")
        else:
            self.world.wait_for_tick()
    ...
    def _load_and_run_scenario(self, config):
        ...
        if not self._load_and_wait_for_world(config.town, config.ego_vehicles):
            ...
            print("[What happended -----]", config.town, config.ego_vehicle)
            self._cleanup()
            
            return False
        #print("[What happended 2]")
    ...
    def load_scenario(self, config):
        ...
        if not self._load_and_wait_for_world(config.town, config.ego_vehicles):
            ...
            return False
        print("world time: " + str(time.perf_counter() - ti))
    ...
    def run_scenario_config(self, config):
        try:
    ...
    def parse_scenarios(self):
        ...
        print("[Scenario_runner configurations]", self.scenario_configurations)
    ...
    def start_scenario(self, config):
        self.finished = False
        self.load_scenario(config)

    def tick(self):
        self.manager.tick_scenario()

    def terminate_scenario(self):
        if self.manager._running:
            self.manager.stop_scenario()
            self.manager.end_scenario()
    ...
    def _run_scenarios(self):
        ...
        scenario_configurations = ScenarioConfigurationParser.parse_scenario_configuration(
            self._args.scenario,
            self._args.configFile)
        print("[Scenario_runner configurations]", scenario_configurations)
