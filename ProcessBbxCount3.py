...
file1_data = read_data('coop3_coop_SERVER.txt')
file2_data = read_data('coop3_coop_YOLO_copy.txt')
file3_data = read_data('coop3_nocoop_SERVER.txt')
file4_data = read_data('coop3_nocoop_YOLO_copy.txt')
...
plt.plot(ticks, file1_data_smooth, label='Groundtruth w/ Coop.', linestyle='-', color=colors[0], linewidth=3)

plt.fill_between(ticks, file3_lower, file3_upper, color=light_blue, alpha=0.1)
plt.plot(ticks, file3_data_smooth, label='Groundtruth w/o Coop. ', linestyle='-', color=light_blue, linewidth=3)


plt.fill_between(ticks, file2_lower, file2_upper, color=colors[1], alpha=0.1)
plt.plot(ticks, file2_data_smooth, label='YOLO w/ Coop.', linestyle='-', color=colors[1], linewidth=3)

plt.fill_between(ticks, file4_lower, file4_upper, color=orange, alpha=0.1)
plt.plot(ticks, file4_data_smooth, label='YOLO w/o Coop.', linestyle='-', color=orange, linewidth=3)

# Add title and labels with larger font size
plt.title('Comparison of the Number of Objects Detected', fontsize=25)
ax = plt.gca()
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
ax.spines['bottom'].set_linewidth(2)  # Set the bottom spine to be bolder
ax.spines['left'].set_linewidth(2)   # Set the right spine to be bolder

fontsize = 22
plt.xlabel('Ticks', fontsize=fontsize)
plt.ylabel('Number of detected objects', fontsize=fontsize)
